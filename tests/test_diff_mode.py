...
P4_DESCRIBE_DIFF = """\
Change 12345 by alice@wks on 2024/01/15 12:00:00

\tTighten authenticate input check

Affected files ...

... //depot/src/auth.py#3 edit

Differences ...

==== //depot/src/auth.py#3 (text) ====
...
def test_parse_p4_describe_style_diff():
    """Recognize Perforce `==== <path>#<rev>` section headers and parse
    the same hunk body format git uses.
    """
    hunks = parse_unified_diff(P4_DESCRIBE_DIFF)
    assert len(hunks) == 1
    h = hunks[0]
    # The depot path comes through as-is at the parser level; P4VCS rewrites
    # to repo-relative before this point in the real pipeline.
    assert h.file_path == "//depot/src/auth.py"
    assert len(h.added_lines) == 3
    assert all(ln >= 7 for ln in h.added_lines)


def test_parse_p4_diff_two_files():
    """Two `====` sections should both be picked up."""
    multi = P4_DESCRIBE_DIFF + """
==== //depot/src/models.py#1 (text) ====
...
"""
    hunks = parse_unified_diff(multi)
    files = {h.file_path for h in hunks}
    assert files == {"//depot/src/auth.py", "//depot/src/models.py"}
